...
namespace BridgeWind
{
	...
	void GeoGenerator::generateGeoFileSingleLoop()  {
		...
		const std::vector<double>& lengthRatios = loop.getLengthRatios();

		std::vector<int> MeshNumbers = proportionalSplit(circumferentialMeshNumber, lengthRatios);
	...
	void GeoGenerator::geoWriteFarfieldPoints(const Loop& loop, int& pointIndex) {
		...
		for (int i = 0; i < nodes.size() - 1; i++) {
			angle += loop.getLengthRatios()[i] * 2 * PI;
			Point point(Point(0, 0), fieldDiameter / 2.0, angle);
			geoSetPoint(pointIndex, point);
			pointIndex++;
		}
		angle += loop.getLengthRatios()[nodes.size() - 1] * 2 * PI;
